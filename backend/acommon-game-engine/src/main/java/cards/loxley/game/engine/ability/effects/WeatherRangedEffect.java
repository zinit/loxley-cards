...
@Component
public class WeatherRangedEffect implements AbilityEffect {

    @Override
    public String code() {
        return AbilityCodes.WEATHER_RANGED;
    }

    @Override
    public void apply(AbilityContext ctx) {
        for (Player p : Player.values()) {
            ctx.state().playerState(p).board().row(RowId.RANGED).applyWeather();
        }
    }
}
